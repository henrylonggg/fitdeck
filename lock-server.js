...
source=source.replace("async function createRoom(host,s,difficulty,game,golfHoles,golfMode){const code=await makeCode(),validGame=['deathbox','lethalcross'].includes(game)?game:'deathbox'","async function createRoom(host,s,difficulty,game,golfHoles,golfMode,golfCpuCount){const code=await makeCode(),validGame=['deathbox','lethalcross','golf'].includes(game)?game:'deathbox'");
source=source.replace("if(validGame==='golf'&&r.golfMode==='ai'){for(let i=1;i<=3;i++){const cpu=newPlayer('CPU '+i,null,null);cpu.connected=true;cpu.cpu=true;r.players.push(cpu)}}","if(validGame==='golf'&&r.golfMode==='ai'){const cpuN=Math.max(1,Math.min(5,Number(golfCpuCount)||3));for(let i=1;i<=cpuN;i++){const cpu=newPlayer('CPU '+i,null,null);cpu.connected=true;cpu.cpu=true;r.players.push(cpu)}}");
source=source.replace("s.on('createRoom',async({name,difficulty,game,golfHoles,golfMode,profileToken}={})=>{try{const n=clean(name);if(!n)return s.emit('errorMessage','Enter your name.');const token=profileToken||s.data.profileToken,pr=await getProfile(token),h=newPlayer(n,s.id,token,pr?.badgeColor,pr?.medals,pr?.xpTotal),r=await createRoom(h,s,difficulty,game,golfHoles,golfMode);","s.on('createRoom',async({name,difficulty,game,golfHoles,golfMode,golfCpuCount,profileToken}={})=>{try{const n=clean(name);if(!n)return s.emit('errorMessage','Enter your name.');const token=profileToken||s.data.profileToken,pr=await getProfile(token),h=newPlayer(n,s.id,token,pr?.badgeColor,pr?.medals,pr?.xpTotal),r=await createRoom(h,s,difficulty,game,golfHoles,golfMode,golfCpuCount);");
source=source.replace("if(r.game==='golf'&&(r.players.length<4||r.players.length>6))return s.emit('errorMessage','Golf requires 4–6 total online players.');","if(r.game==='golf'&&(r.players.length<2||r.players.length>6))return s.emit('errorMessage','Golf requires 2–6 total players.');");
source=source.replace("if(!g.ai){const placement=[900,600,350][rank-1]||150;const bonus=Math.min(150,g.holesWon[i]*15)+(total<g.holes*4?100:0);const earned=placement+bonus;p.xpEarned+=earned;p.xpTotal+=earned;if(p.profileToken){const pr=await getProfile(p.profileToken);if(pr){pr.xpTotal=p.xpTotal;await saveProfile(pr)}}} ","if(false){const earned=0;} ");
...
const replacement=`function sendApp(req,res){
 fs.readFile(path.join(__dirname,'public','index.html'),'utf8',(err,html)=>{
  ...
  const golfInlineMarker='(function(){function golfCard';
  const golfInlineClose='</script>';
  const golfInlineMarkerAt=html.indexOf(golfInlineMarker);
  if(golfInlineMarkerAt>=0){
   const golfInlineAt=html.lastIndexOf('<script>',golfInlineMarkerAt);
   const golfInlineEndAt=html.indexOf(golfInlineClose,golfInlineMarkerAt);
   if(golfInlineAt>=0&&golfInlineEndAt>=0)html=html.slice(0,golfInlineAt)+html.slice(golfInlineEndAt+golfInlineClose.length);
  }
