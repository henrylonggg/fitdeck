(function(){
  ...
  function initials(){var c=clerk(),u=c&&c.user;var full=String((u&&(u.fullName||u.username||u.firstName||u.lastName))||'');var email=String((u&&u.primaryEmailAddress&&u.primaryEmailAddress.emailAddress)||'');var source=(full||email.split('@')[0]||'Player').replace(/[_\.\-]+/g,' ').trim();var parts=source.split(/\s+/).filter(Boolean);var one=(parts[0]||'P').slice(0,1);var two=(parts.length>1?parts[parts.length-1].slice(0,1):(parts[0]||'').slice(1,2));return (one+two).toUpperCase()||'P'}
  function installCss(){if($('authHardResetStyle'))return;var s=document.createElement('style');s.id='authHardResetStyle';s.textContent='\
    ...
    #deathboxNav [data-nav="profile"] .cl-rootBox,#deathboxNav [data-nav="profile"] .cl-userButtonBox,#deathboxNav [data-nav="profile"] .cl-avatarBox,#deathboxNav [data-nav="profile"] small,#deathboxNav [data-nav="profile"] img{display:none!important}\
    .db-clerk-avatar-host{width:38px!important;height:38px!important;min-width:38px!important;border-radius:50%!important;display:grid!important;place-items:center!important;background:linear-gradient(145deg,#f1c969,#ff4055)!important;color:#050505!important;font-size:13px!important;font-weight:1000!important;letter-spacing:.04em!important;overflow:hidden!important;box-shadow:0 8px 22px rgba(0,0,0,.35)!important}\
  ';document.head.appendChild(s)}
  function avatarMarkup(){return '<span class="db-clerk-avatar-host" data-initials="'+esc(initials())+'">'+esc(initials())+'</span>'}
  ...
  function clean(){installCss();var chip=$('clerkUserChip');if(chip)chip.remove();var panel=$('profilePanel');if(panel){panel.classList.add('hidden');panel.style.display='none'}var details=$('profileDetails');if(details){details.classList.add('hidden');details.style.display='none'}stripLegacyGate();var btn=document.querySelector('#deathboxNav [data-nav="profile"]');if(btn&&authed()){var text=initials(),host=btn.querySelector('.db-clerk-avatar-host');if(!host||host.dataset.initials!==text||btn.childElementCount!==1){btn.innerHTML=avatarMarkup()}if(!btn.__authHardAvatar){btn.__authHardAvatar=true;btn.addEventListener('click',openClerk,true);btn.addEventListener('pointerup',openClerk,true);btn.addEventListener('touchend',openClerk,true)}}}
