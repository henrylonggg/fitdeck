(function(){
  ...
  function enforceControls(){var s=stateSafe(),dir=byId('directionChoices'),colors=byId('colorChoices');if(!s)return;if(s.game==='deathbox'){if(colors){colors.classList.add('hidden');colors.style.display='none';colors.style.visibility='hidden'}if(dir){dir.classList.remove('hidden');dir.style.display='grid';dir.style.visibility='visible'}}}
  function render(){
    style();removeOldTrackers();enforceControls();
