(function(){
  var locked=false,lockedGame='',scrollY=0,lastRoom=null,wiredSocket=null,lastToggleAt=0;
  ...
  function css(){if($('finalGameUiFixStyle'))return;var s=document.createElement('style');s.id='finalGameUiFixStyle';s.textContent='\
    ...
    #gameSection .turnbar .game-lock-toggle~.game-lock-toggle{display:none!important}\
    ...
    html:has(body.screen-lock-active),body.screen-lock-active{background:#030405!important;background-image:none!important;overflow:hidden!important}\
    body.screen-lock-active:before,body.screen-lock-active:after{display:none!important}\
    body.screen-lock-active #deathboxNav{display:none!important}\
    body.screen-lock-active .app{background:#030405!important;max-width:none!important;width:100vw!important;min-height:100svh!important;margin:0!important;padding:0!important}\
    body.screen-lock-active #gameSection{position:fixed!important;inset:0!important;display:block!important;visibility:visible!important;opacity:1!important;background:#030405!important;overflow:hidden!important;padding:0!important;margin:0!important;z-index:900!important}\
    body.screen-lock-active #gameSection .board-panel{height:100svh!important;min-height:100svh!important;width:100vw!important;margin:0!important;border-radius:0!important;padding:10px!important;overflow:hidden!important;background-color:#050507!important}\
    body.screen-lock-active.theme-lethalcross #gameSection .board-panel{background:radial-gradient(circle at 50% 0,rgba(74,168,255,.19),transparent 22rem),linear-gradient(145deg,#061b35,#03070c)!important}\
    body.screen-lock-active.theme-deathbox #gameSection .board-panel{background:radial-gradient(circle at 50% 0,rgba(255,64,85,.20),transparent 22rem),linear-gradient(145deg,#251015,#050506)!important}\
  ...
  function currentGame(){if(lastRoom)return gameOf(lastRoom);var board=$('board');if(board&&board.classList.contains('lethalcross'))return 'lethalcross';return 'deathbox'}
  function ensureButton(){var turn=document.querySelector('#gameSection .turnbar');if(!turn)return null;var buttons=Array.from(turn.querySelectorAll('.game-lock-toggle,[data-game-lock],[data-final-game-lock]'));var b=buttons[0];if(!b){b=document.createElement('button');b.type='button';b.className='game-lock-toggle';turn.appendChild(b)}buttons.slice(1).forEach(function(x){x.remove()});b.className='game-lock-toggle';b.dataset.finalGameLock='1';b.dataset.gameLock=currentGame();b.textContent='';b.setAttribute('aria-label',locked?'Unlock screen':'Lock screen');b.title=locked?'Unlock screen':'Lock screen';b.classList.toggle('on',locked);return b}
  function escapeButton(){var b=$('lockModeEscapeButton');if(!b){b=document.createElement('button');b.type='button';b.id='lockModeEscapeButton';document.body.appendChild(b)}b.textContent='';b.setAttribute('aria-label','Unlock screen');b.title='Unlock screen';b.classList.toggle('on',locked);b.classList.toggle('show',locked);return b}
  ...
  function freeze(game){scrollY=window.scrollY||document.documentElement.scrollTop||0;lockedGame=game||currentGame();locked=true;document.body.classList.remove('screen-lock-deathbox','screen-lock-lethalcross');document.body.classList.add('screen-lock-active','screen-lock-'+lockedGame);document.body.style.position='fixed';document.body.style.top='-'+scrollY+'px';document.body.style.left='0';document.body.style.right='0';document.body.style.width='100%';refresh()}
  function toggle(){var now=Date.now();if(now-lastToggleAt<450)return;lastToggleAt=now;if(locked)return release();freeze(currentGame())}
  ...
  function handleClick(e){var t=e.target;if(!t||!t.closest)return;if(t.closest('#lockModeEscapeButton')||t.closest('[data-final-game-lock]')||t.closest('#gameSection .game-lock-toggle')){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();toggle();return false}}
